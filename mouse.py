...
class ArduinoMouse:
    def __init__(self, filter_length=3):
        ...
        try:
            ...
            print(colored('[Error]', 'red'), colored('Colorant is already open or serial port in use by another app. Close Colorant and other apps before retrying.', 'white'))
            time.sleep(10)
            sys.exit()

    def find_serial_port(self):
        port = next((port for port in serial.tools.list_ports.comports() if "USB" in port.description), None)
        if port is not None:
            return port.device
        else:
